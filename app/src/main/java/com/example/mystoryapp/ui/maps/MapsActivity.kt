...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun setupViewModel() {
        ...
        mapsViewModel.getUser().observe(this, { user ->
            if (user.isLogin){
                mapsViewModel.setstory(token = user.token)
        ...
        mapsViewModel.story().observe(this){story ->
            val storyItem = story?.map {  LatLng(it.lat,it.lon) }
            if (storyItem != null) {
                for(i in 0 until story.size)
                    storyItem.forEachIndexed{index,LatLng ->
                        mMap.addMarker(
                            MarkerOptions()
                                .position(LatLng)
                                .title(story[index].name)
                                .snippet(story[index].description)
                )}
            }
        }
    }
